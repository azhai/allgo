// Package config provides a simple environment variable manager for Go.
...
// New creates a new Environ instance and loads environment variables from the default .env file.
func New() *Environ {
	return NewWithFile(".env")
}

// NewWithFile creates a new Environ instance and loads environment variables from the specified file.
// It initializes the storage map and attempts to Load the file.
// If the file cannot be opened or read, the error is ignored.
func NewWithFile(filename string) *Environ {
